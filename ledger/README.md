...
- **記一筆**：輸入金額 + 摘要 + 收/支，系統**自動建議借貸方向與科目**；不對就改，系統會**學起來**，下次自動套用（規則式學習，可解釋、不靠黑盒）。
- **現金 + 應計**：平時記銀行收支（現金基礎）；勾「應收/應付」即產生應計分錄，之後再收付款。
- **三大報表**：損益表、資產負債表、現金流量表（直接法）。
- **金額一律整數元**，複式分錄一律借貸平衡（有自動驗證 + 單元測試）。
